...
- **All shell access, version comparison, and status classification live in the Rust backend.** The frontend never shells out. It only calls `invoke()`. Its only version-adjacent logic is the appetite dial's safe/held mapping over the backend-computed bump kind. If you need a new capability, add a Tauri command, do not run a process from JavaScript.
